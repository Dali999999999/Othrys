...
- Sensitive credentials (passwords, private SSH keys, passphrases) and host identity stores are encrypted using authenticated **AES-256-GCM**.
- **Master Key**: A 256-bit high-entropy cryptographic key generated by CSPRNG is persisted within OS-level protected credential stores (Windows DPAPI, macOS/iOS Keychain, Android Keystore, Linux Secret Service) via `FlutterSecureStorage`.
- **Per-Record Key Diversification**: Subkeys for each encrypted item are derived from the master key using **PBKDF2** (HMAC-SHA256, **100,000 rounds**) with a unique, cryptographically random 16-byte salt per payload.
- **Authenticated Integrity**: Each ciphertext payload embeds its unique salt, a fresh 96-bit CSPRNG nonce, and an authentication tag (MAC) to guarantee confidentiality and tamper resistance.
- **RAM Zeroization**: In-memory master key buffers are explicitly overwritten with zeroes upon application shutdown.
